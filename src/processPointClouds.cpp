...
ProcessPointClouds<PointT>::SeparateClouds(
  ...
  for (int index : inliers->indices) {
    planeCld->points.push_back(cloud->points[index]);
  }
  pcl::ExtractIndices<PointT> extract;
  ...
  std::pair<typename pcl::PointCloud<PointT>::Ptr, typename pcl::PointCloud<PointT>::Ptr> segResult(planeCld, obstCld);
  return segResult;
}
...
ProcessPointClouds<PointT>::SegmentPlane(
  ...
  if (inliers->indices.size() == 0) {
    std::cout << "Could not estimate planer model for the given data " << std::endl;
  }
  ...
  std::pair<typename pcl::PointCloud<PointT>::Ptr, typename pcl::PointCloud<PointT>::Ptr> sepResults = SeparateClouds(
    inliers, cloud);
  return sepResults;
}
...
ProcessPointClouds<PointT>::Clustering(typename pcl::PointCloud<PointT>::Ptr cloud,
  ...
  for (const pcl::PointIndices &getIndices: clusterIndices) {
    ...
    for (int ii: getIndices.indices)
      cloudCluster->points.push_back(cloud->points[ii]);
...
template<typename PointT>
std::pair<typename pcl::PointCloud<PointT>::Ptr, typename pcl::PointCloud<PointT>::Ptr>
ProcessPointClouds<PointT>::Ransac3D(
  typename pcl::PointCloud<PointT>::Ptr cloud,
  int maxIterations,
  float distanceTol) {
  ...
  // For max iterations
  for (auto idx = 0; idx != maxIterations; ++idx) {
    // radomly pick two numbers
    ...
    // calculate the distance and find the outliers
    for (auto ii = 0; ii != cloud->points.size(); ++ii) {
      if (inliers.count(ii) > 0)
        continue;

      float x4 = cloud->points[ii].x;
      float y4 = cloud->points[ii].y;
      float z4 = cloud->points[ii].z;
  ...
  for (int index = 0; index < cloud->points.size(); index++) {
    if (inliersResult.count(index))
      ...
      obstCld->points.push_back(cloud->points[index]);
  }
  std::pair<typename pcl::PointCloud<PointT>::Ptr, typename pcl::PointCloud<PointT>::Ptr> segResult(planeCld, obstCld);
  return segResult;
}
...
void ProcessPointClouds<PointT>::clusterHelper(
  ...
  if ((processed[idx] == false) && (cluster.size() < maxSize)){
    processed[idx] = true;
...
std::vector<typename pcl::PointCloud<PointT>::Ptr> ProcessPointClouds<PointT>::euclideanCluster(
  ...
  for (size_t idx = 0; idx < cloud->points.size(); ++idx) {
    ...
    if (processed[idx] == false) {
      ...
      clusterHelper(idx, cloud, cluster_idx, processed, tree, distanceTol, maxSize);

      if ((cluster_idx.size() >= minSize) && (cluster_idx.size() <= maxSize)) {
        cluster_indices.push_back(cluster_idx);
      }
      }
    }
  ...
  for (const auto & cluster_indice : cluster_indices) {
    ...
    for ( const auto & index : cluster_indice) {
